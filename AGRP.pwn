/*******************************************************************************
This script is entirely coded by Vegeta.
...
//==============================================================================
	...
	CreateObject(9623,-151.44289, 576.12183,17.87950,0.00000,0.00000,99.72005);
 	CreateObject(3799,2483.7100,1047,2980,0.00,0.00,0.00);
 	CreateObject(18662,2002.13940,-1114.74890,27.17810,0.00000,0.00000,99.95998);
 	CreateObject(18750,1629.74756,-2299.69751, 81.18016,-263.28000,1.32001,-3.12000);
	CreateObject(3800,2486.5828,1045.1124,2980,0.00,0.00,0.00);
	CreateObject(18750, -1436.49719, -275.98792, 50.63769,   85.80001, 0.24000, -29.75999);
	CreateObject(9623, -1903.66394, -534.77777, 40.01428,   1.32000, 0.24000, 0.36000);
	CreateObject(9623, 50.78437, -1531.87109, 7.10661,   0.00000, 0.00000, -99.24002);
...
public OnPlayerPickUpDynamicPickup(playerid,pickupid)
{
	...
	if(pickupid==bloodring )
	{
        ...
        SendClientMessage(playerid,YELLOW,"Welcome to Sand King");
		SetPlayerInterior(playerid,4);
	}
	if(pickupid==sandking)
	{
		SetPlayerPos(playerid,-1424.6554,931.4493,1036.4042);
        SendClientMessage(playerid,YELLOW,"Welcome to Blood Ring");
	...
	{
        SendClientMessage(playerid,YELLOW,"Welcome to LS PaintBall");
	...
	for(new i=1;i<PropertyCount+1;i++)
	{
        if(pickupid==Property[i][pPickup])
		{
        	...
        	{
            	format(string,sizeof(string),"This Property is owned by {FFFFFF}'%s'",Property[i][pOwner]);
   				SendClientMessage(playerid,YELLOW,string);
        	}
        	if(strcmp(Property[i][pOwner],"None")==0)
        	{
        		format(string,sizeof(string),"This Property is for sale,Price: $%i",Property[i][pCost]);
   				SendClientMessage(playerid,YELLOW,string);
   				format(string,sizeof(string),"~w~FOR SALE: $%d\nTO BUY USE /BUYPROP",Property[i][pCost]);
...
public OnPlayerEnterDynamicCP(playerid,checkpointid)
{
	if(checkpointid==dl[0]||checkpointid==dl[1]||checkpointid==dl[2]||checkpointid==dl[3]||checkpointid==dl[4]||checkpointid==dl[5]||checkpointid==dl[6]||checkpointid==dl[7]||checkpointid==dl[8]||checkpointid==dl[9]||checkpointid==dl[10]||checkpointid==dl[11])
	{
	    ...
	    if(GetPlayerVehicleID(playerid)!=Car[playerid])return SendClientMessage(playerid,GREY,"This vehicle isn't owned by you");
		SetVehicleVelocity(GetPlayerVehicleID(playerid),0,0,0);
  		SendClientMessage(playerid,YELLOW,"Welcome to your local San Andreas Car Dealers");
		ShowPlayerDialog(playerid,CARDEAL,DIALOG_STYLE_LIST,"San Andreas Dealership","1. Vehicle Insurance\n2. Purchase Auto-Lock\n3. Set Number Plate\n4. Change Color\n5. Sell Car\n6. Get Quote\n7. Vehicle Alarm System","Select","Close");
	}
	...
	if(checkpointid==dlcp)
	{
	    if(DL[playerid]==0)
		{
				ShowPlayerDialog(playerid,FIRSTDIALOG,DIALOG_STYLE_LIST,"Driving School","\nPractical Test","Select","Cancel");
   		}
		else SendClientMessage(playerid,GREY,"You already own a driving license");
	}
	if(checkpointid==flpickup)
	{
	    if(FL[playerid]==0)
		{
     			if(GetPlayerScore(playerid)==0)return SendClientMessage(playerid,-1,"You need a score of at least level 1");
				SendClientMessage(playerid,YELLOW,"Type /taketest to go through your flying license test");
   		}
		else SendClientMessage(playerid,GREY,"You already own a flying license");
	}
	if(checkpointid==blpickup)
	{
	    if(BL[playerid]==0)
		{
  			SendClientMessage(playerid,YELLOW,"Type /taketest to go through your bike license test");
   		}
		else SendClientMessage(playerid,GREY,"You already own a bike license");
	}
	if(checkpointid==blcp)
	{
	    if(BtL[playerid]==0)
		{
			SendClientMessage(playerid,YELLOW,"Type /boatlicense to get your boat licenses");
		}
		else SendClientMessage(playerid,GREY,"You already own a boat license");
	}
	for(new i=1;i<OrgsCount+1;i++)
	{
		...
		{
            new string[50];
            format(string,sizeof(string),"Organization: %s",GetOrgName(i));
			...
			if(PlayerOrg[playerid]!=i)
			{
			    ...
			    {
					ShowPlayerDialog(playerid,ORGCP,DIALOG_STYLE_LIST,string,"Request\nMembers\nTurfs","Ok","Cancel");
				}
			    if(strcmp(Organization[i][Turf],"None")==0)
			    {
					ShowPlayerDialog(playerid,ORGCP,DIALOG_STYLE_LIST,string,"Request\nMembers","Ok","Cancel");
            ...
            if(PlayerLeader[playerid]==0&&PlayerOrg[playerid]==i)
			{
    			if(strcmp(Organization[i][Turf],"None")!=0)
			    {
					ShowPlayerDialog(playerid,ORGCP,DIALOG_STYLE_LIST,string,"Information\nMembers\nCollect Wage\nSuit Up\nTurfs","Ok","Cancel");
				}
    			if(strcmp(Organization[i][Turf],"None")==0)
			    {
					ShowPlayerDialog(playerid,ORGCP,DIALOG_STYLE_LIST,string,"Information\nMembers\nCollect Wage\nSuit Up","Ok","Cancel");
	...
	for(new i=1;i<BusinessCount+1;i++)
	{
        if(checkpointid==Business[i][bCheckpoint])
		{
        	...
        	{
        	    if(INI_Int(Acc(playerid),"Profit")<1)return SendClientMessage(playerid,-1,"Welcome back to your business. Earnings: $0");
        	    format(str,sizeof(str),"{FFFFFF}Do you want to collect the profit of $%i from your business?",INI_Int(Acc(playerid),"Profit"));
        	...
        	{
            	format(string,sizeof(string),"Business: %s,owned by {FFFFFF}'%s'",Business[i][bName],Business[i][bOwner]);
   				SendClientMessage(playerid,YELLOW,string);
   				if(Business[i][bProtected]>0)
   				{
   					format(string,sizeof(string),"This business is protected by %s",GetOrgName(Business[i][bProtected]));
   					SendClientMessage(playerid,RED,string);
   				}
        	}
        	if(strcmp(Business[i][bOwner],"None")==0)
        	{
        		format(string,sizeof(string),"'%s' is for sale,Price: $%i",Business[i][bName],Business[i][bCost]);
   				SendClientMessage(playerid,YELLOW,string);
   				format(string,sizeof(string),"~w~FOR SALE: $%d\nTO BUY USE /BUYBUS",Business[i][bCost]);
   				GameTextForPlayer(playerid,string,3000,5);
        	}
		}
    }
...
public OnPlayerText(playerid,text[])
{
	if(Mute[playerid]>0)
	{
	    SendClientMessage(playerid,RED,"ERROR: {FFFFFF}You are muted");
	    return 0;
	}
    	    	...
    	    	if(PlayerOrgType(a)==LAW)
    	   		{
    				...
    				format(str,sizeof(str),"Reported From Location: %s",zone);
		...
		if(strlen(text)==strlen(ChatLine[playerid]) && !strcmp(ChatLine[playerid],text,false))
		{
		  	...
		  	if(Chance[playerid]==4)
		  	{
		  	    new str[128];
   				Mute[playerid]=3;
                Chance[playerid]=0;
   				format(str,128,"%s has been muted for repeatedly sending messages",PlayerName(playerid));
				...
				if(PlayerAwards[playerid][Shutup]==0)
				{
		    		...
		    		SendClientMessage(playerid,YELLOW,"Achievement Unlocked: {FFFFFF}Will You Shut Up?");
  					SendClientMessage(playerid,LGREEN,"BONUS: $1500 || +70 Gamer Score");
				}
				return 0;
		  	}
		}
	...
	if(strfind(text,"Kainat",true,0)!=-1)
	{
		if(PlayerAwards[playerid][Kainat]==0)
		{
		    ...
		    SendClientMessage(playerid,YELLOW,"Achievement Unlocked: {FFFFFF}Secret Award!");
  			SendClientMessage(playerid,LGREEN,"BONUS: $5000 || +80 Gamer Score");
		}
	}
    ...
    for(new i=0; i<MAX_PLAYERS; i++)
    {
        if(IsPlayerConnectedEx(i))
        {
    		...
    		if(!strcmp(ip,pip,true))
    		{
				...
				if(success)
    			{
    			    ShowPlayerDialog(i,SVPASS,DIALOG_STYLE_PASSWORD,"{FF0000}Remote Console","{FFFFFF}Advance Gaming RolePlay Remote Console\n\nPlease Enter Remote Console Password","Enter","");
			    }
    		}
		}
	 }
...
public OnPlayerRequestClass(playerid,classid)
{
	if(IsLogged[playerid]==0)
	{
		...
		{
			format(string,128,"{FFFFFF}Welcome {A9C4E4}%s{FFFFFF},enter your password below to login to your account",PlayerName(playerid));
			ShowPlayerDialog(playerid,LOGIN,DIALOG_STYLE_PASSWORD,"Login Menu",string,"Login","");
		}
		if(!fexist(Acc(playerid)))
    	{
    	    format(string,128,"{FFFFFF}Welcome {A9C4E4}%s{FFFFFF},enter your password below to register this account",PlayerName(playerid));
...
public OnDialogResponse(playerid,dialogid,response,listitem,inputtext[])
{
			...
			if(strcmp(inputtext,"changeme",false)==0)
			{
        		PlayerPlaySound(playerid,1057,0.0,0.0,0.0);
     			...
     			if(AdminLevel[playerid]!=4)
     			{
     				...
     				SendClientMessage(playerid,-1,"You have been set has Head Admin,automatically");
				}
			}
  			if(strcmp(inputtext,"changeme",false) !=0)
  			{
     			SendClientMessage(playerid,RED,"SERVER: {FFFFFF}Access Denied");
			 	ShowPlayerDialog(playerid,SVPASS,DIALOG_STYLE_PASSWORD,"{FF0000}Remote Console","{FFFFFF}Advance Gaming RolePlay Remote Console\n\nPlease Enter Remote Console Password","Enter","");
			}
		}
		return 1;
	}
	if(dialogid==REGISTER)
	{
		...
		format(string,128,"{FFFFFF}Welcome {A9C4E4}%s{FFFFFF},enter your password below to register account",PlayerName(playerid));
		if(strlen(inputtext)<6)return
		SendClientMessage(playerid,LRED,"The password needs to contain atleast 6 characters/letters")&&
 		ShowPlayerDialog(playerid,REGISTER,DIALOG_STYLE_INPUT,"Registration",string,"Register","");
		IsLogged[playerid]=1;
		...
		format(string,128,"You succesfully registered the account {FFFF00}'%s' {FFFFFF}with the password {FFFF00}'%s'",PlayerName(playerid),inputtext);
		SendClientMessage(playerid,-1,string);
		return 1;
	}
	if(dialogid==LOGIN)
	{
      	...
      	format(str1,128,"{FFFFFF}Welcome {A9C4E4}%s{FFFFFF},enter your password below to login to your account",PlayerName(playerid));
      	if(!response) return Kick(playerid);
		if(strlen(inputtext)<6)return SendClientMessage(playerid,LRED,"The password needs to contain atleast 6 characters/letters")&&
		...
		if(udb_hash(inputtext)==tmp)
	  	{
			...
			SendClientMessage(playerid,-1,"You've logged in,your stats have been restored successfully");
  			if(VIP[playerid]>0)
			{
				SendClientMessage(playerid,-1,"{A9C4E4}Account Status : {FFFFFF}Premium Account");
			}
      	...
      	else
      	{
  			...
  			format(str1,128,"{FFFFFF}Welcome {A9C4E4}%s{FFFFFF} enter your password below to login to your account",PlayerName(playerid));
	  		ShowPlayerDialog(playerid,LOGIN,DIALOG_STYLE_PASSWORD,"Login Menu",str1,"Login","");
	  		SendClientMessage(playerid,RED,"SERVER: {FFFFFF}Access Denied. One more try left");
      	}
		return 1;
	}
	if(dialogid==JOBS)
	{
		...
		switch(listitem)
		{
			...
			{
       			Job[playerid]=0;
			   	SendClientMessage(playerid,-1,"You are now civilian,you have no job");
			}
		}
	}
	if(dialogid==PASSPORT)
	{
		...
		switch(listitem)
		{
			case 0:
			{
        	   ...
        	   Passport[playerid]=1;
			   SendClientMessage(playerid,YELLOW,"You've purchased a passport of Los Santos");
			}
			case 1:
			{
			    if(GetPlayerMoneyEx(playerid) < 1000) return SendClientMessage(playerid,GREY,"You don't have enough money on you");
        	    ...
        	    GivePlayerMoneyEx(playerid,-1000);
			    Passport[playerid]=2;
			   	SendClientMessage(playerid,YELLOW,"You've purchased a passport of Las Venturas");
			}
			case 2:
			{
			    if(GetPlayerMoneyEx(playerid) < 1000) return SendClientMessage(playerid,GREY,"You don't have enough money on you");
        	   	...
        	   	GivePlayerMoneyEx(playerid,-1000);
       			Passport[playerid]=3;
			   	SendClientMessage(playerid,YELLOW,"You've purchased a passport of San Fierro");
			}
		}
	}
	if(dialogid==BANK)
	...
	if(dialogid==WITHDRAW)
	{
		...
		if(response)
	    {
			...
			{
				ShowPlayerDialog(playerid,WITHDRAW,DIALOG_STYLE_INPUT,"San Andreas Bank","{FFFFFF}Please Enter The Amount,You would like to withdraw","Withdraw","Back");
			}
			else
			{
				GivePlayerMoneyEx(playerid,strval(inputtext));
				Bank[playerid]=Bank[playerid] - strval(inputtext);
				format(string,sizeof(string),"You have withdrawn $%d from your Bank Account. New Balance: $%d",strval(inputtext),Bank[playerid]);
				SendClientMessage(playerid,YELLOW,string);
	...
	if(dialogid==DEPOSIT)
	{
		...
		if(response)
	    {
			...
			{
                ShowPlayerDialog(playerid,DEPOSIT,DIALOG_STYLE_INPUT,"San Andreas Bank","{FFFFFF}Please Enter The Amount,You would like to deposit","Deposit","Back");
			}
			else
			{
				Bank[playerid]=Bank[playerid] + strval(inputtext);
				GivePlayerMoneyEx(playerid,-strval(inputtext));
                format(string,sizeof(string),"You have deposited $%d,your new balance is $%d",strval(inputtext),Bank[playerid]);
                SendClientMessage(playerid,YELLOW,string);
                TogglePlayerControllable(playerid,1);
  	...
  	if(dialogid==ATM)
	{
		...
		if(response)
	    {
			...
			if(strval(inputtext) > 5000)return ShowPlayerDialog(playerid,ATM,DIALOG_STYLE_INPUT,"San Andreas Bank","{FFFFFF}You cannot withdraw more than $5000\nPlease Enter The Amount,You would like to withdraw","Withdraw","Back");
			GivePlayerMoneyEx(playerid,strval(inputtext));
			Bank[playerid]=Bank[playerid] - strval(inputtext);
...
{
	if(IsJailedOrMuted(playerid))return 1;
    SendClientMessage(playerid,YELLOW,"Advance Gaming Roleplay Server Help");
    ...
    SendClientMessage(playerid,-1,"If you need any help,do not hesitate in anyway to ask,type /url for Advance Gaming Roleplay/RealLife Forums");
    return 1;
}
//==============================================================================
			...
			if(AdminLevel[i] > 0)
			{
			    if(Hidden[i]==0)
		    	{
     				...
     				if(AdminLevel[i]==2){name="{0000FF}Global Moderator {FFFFFF}(Level 2)";}
           			...
           			if(AdminLevel[i]==4){name="{ADFF2F}Lead Admin {FFFFFF}(Level 3+)";}
           			if(AdminLevel[i]==5){name="{FF0000}Head Admin{FFFFFF}(Level 4+)";}
...
{
	if(IsJailedOrMuted(playerid))return 1;
	SendClientMessage(playerid,YELLOW,"***Animations***");
